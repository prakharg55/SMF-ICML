...
#SBATCH --job-name=eval_med_add_sparse
...
python -m smf_rebuild.eval_tasks \
  ...
  --memory-checkpoint "$SMF_MED_RUN_DIR/qwen_additive_memory_sparse_kl" \
  --tasks medmcqa,wikitext,triviaqa \
  --limit 1000 \
  --output "$SMF_MED_RUN_DIR/eval_additive_sparse_kl.json"
